...
const HistoryLoader: React.FC<{ count: number }> = ({ count }) => {
  return (
    <div>
      <div className="overflow-y-auto">
        {[...Array(count)].map((_, index) => (
          <>
            <Skeleton className="w-32 h-5 my-2 bg-muted-foreground/30" />
            <div
              key={index}
              className="flex items-center py-2 lg:my-2 bg-transparent"
            >
